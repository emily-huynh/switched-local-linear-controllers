...
def my_legendHandler():
    ...
    space = '\n'
    black_line = mlines.Line2D([], [], ls='-', color='black', label=f'Position of{space}Vehicle')
    return blue_line, magenta_line, black_line

def segment_LegendHandler():
    blue_line = mlines.Line2D([], [], ls='none', marker = "o", color='deepskyblue', alpha = 0.5, label='K1')
    magenta_line = mlines.Line2D([], [], ls='none',marker = "o",  color='fuchsia', alpha = 0.5, label='K2')
    green_line = mlines.Line2D([], [], ls='none',marker = "o", color='lime', alpha = 0.5, label='K3')
    red_line = mlines.Line2D([], [], ls='none', marker = "o", color='darkorange', alpha = 0.5, label='K4')
    space = '\n'
    black_line = mlines.Line2D([], [], ls='--', color='black', label=f'Road{space}Edges')
    return blue_line, magenta_line, green_line, red_line, black_line

def plot_segments(a, b, c, ax3):
    ...
    # Fill regions 
    ax3.fill_between((x[0],x[1]), y1, y12, where=(y12 >= y1), color = 'deepskyblue', alpha = 0.25, interpolate = False)
    ax3.fill_between(x, y2, y22, where=(y2 >= y22), color = 'fuchsia', alpha = 0.25*.7, interpolate = False)
    ax3.fill_between((x[1],x[2]), y3, y32, where=(y3 >= y32), color = 'lime', alpha = 0.25, interpolate = False)
    ax3.fill_between(x, y4, y42, where=(y4 >= y42), color = 'darkorange', alpha = 0.25, interpolate = False)
    ax3.grid(which='both',  linestyle='--') 

def plot_segmentsTimeSeries(t, graph_positionColour, minY, maxY, ax, alphaNumber):
    
    current_colour = graph_positionColour[0]
    
    whenToColour = [0]
    for i in range(len(t)):
        this_colour = graph_positionColour[i]
        if current_colour != this_colour:
            whenToColour.append(i)
            current_colour = this_colour
    
    ax.fill_between(t[whenToColour[0]:whenToColour[1]], minY, maxY, color = 'deepskyblue', alpha = alphaNumber)
    ax.fill_between(t[whenToColour[1]:whenToColour[2]], minY, maxY, color = 'fuchsia', alpha = alphaNumber)
    ax.fill_between(t[whenToColour[2]:whenToColour[3]], minY, maxY, color = 'lime', alpha = alphaNumber)
    ax.fill_between(t[whenToColour[3]:whenToColour[4]], minY, maxY, color = 'darkorange', alpha = alphaNumber)
    ax.fill_between(t[whenToColour[4]:], minY, maxY, color = 'deepskyblue', alpha = alphaNumber)
           
''' Road Functions '''
def road_maxmin(roadlist, roadGeometries):
...
def plot_decalRoad(ax, roadlist, roadGeometries, title):
...
def plot_road(ax, roadlist, title, left_colourAndtype, right_colourAndtype, roadGeometries, roadimported):
...
def plot_speed(axS, wheelSpeed, formatSpeed):
    ...
    axS.set_xlim([0-0.5,t[-1]+0.5])
    axS.set_ylim([0,40])
    axS.grid(which='both',  linestyle='--') 
    
def plot_throttle(axT, throttle, formatThrot):
...
def plot_steering(axSt, steering, formatSteer):
...
def plot_brake(axB, brakes, formatBrake):
...
def plot_headingVectors(axD, x_head, y_head):
...
def calc_xFromCentreMode(totalTime, All_zeta, ID_List, roadGeometries, plotMode):
    # Inits
    ...
    if plotMode:
        ...
        [road_leftEdges, road_rightEdges, roadGeometries] = plot_road(ax, roadlist, f' ', 'k--', 'k-.', roadGeometries, True)
        ...
        centreline = mlines.Line2D([], [], ls='none', color='magenta', marker = ".", label='Centreline')
        vehiclePos = mlines.Line2D([], [], ls='none', color='blue', marker = ".", label='Position of Vehicle')
        roadEdge = mlines.Line2D([], [], ls='--', color='black', label='Road Edges')
        
        plt.legend(handles=[centreline,vehiclePos,roadEdge],loc='lower right')
        plt.show()
